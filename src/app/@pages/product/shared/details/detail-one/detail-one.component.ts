...
export class DetailOneComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const urlImagen = 'assets/images/brands/' + this.product.brand + '.jpg';
    this.imagenExiste(urlImagen, (existe) => {
      if (existe) {
  ...
  imagenExiste(url: string, callback: (existe: boolean) => void) {
    const img = new Image();
    img.onload = () => {
  ...
  handleImageError() {
    // En caso de error al cargar la imagen, establece una imagen alternativa
    this.product.brand = 'na';
  }
